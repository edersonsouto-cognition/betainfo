...
1. Measure coverage (`pytest-cov` is not a project dependency, so pull it in for the run):
   ...
   ```
2. Pick the most critical uncovered paths first. This repo intentionally omits `TestUpdateTask`, `TestSearchTasks`, `TestGetOverdueTasks` and `TestGetStats` in `tests/test_task_service.py` — start there when the task is generic "improve coverage".
...
6. Tests must assert the *correct* behavior. Several functions in `src/services/task_service.py` carry `# BUG:` comments — if a correct test fails, report the bug rather than asserting the buggy behavior, and only fix the code if the task asks for it.
