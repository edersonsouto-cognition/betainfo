---
name: pre-pr-check
description: Quality gate to run before opening any PR in this repo. Runs ruff and pytest (the same checks as CI), then self-reviews the diff.
---
...
2. Run the test suite; every test must pass:
   ...
   uv run pytest
   ```
3. Read the full diff against the base branch: `git diff --merge-base origin/main`.
...
7. Only open the PR once lint and tests pass. These are exactly the two CI jobs in `.github/workflows/ci.yml` (lint, then test), so a green local run means a green CI run.
